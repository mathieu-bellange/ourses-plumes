...
(function () {
	...
	CKEDITOR.plugins.add("autosave", {
		lang: 'de,en,fr,jp,pl,pt-br,sv,zh,zh-cn', // %REMOVE_LINE_CORE%
		version : 0.11,
		init : function (editor) {
			//CKEDITOR.document.appendStyleSheet(CKEDITOR.getUrl(CKEDITOR.plugins.getPath('autosave') + 'css/autosave.min.css'));
			CKEDITOR.document.appendStyleSheet(CKEDITOR.getUrl(CKEDITOR.plugins.getPath('autosave') + 'css/autosave.css')); // EDIT
			...
			return {
				...
				contents : [{
						...
						elements : [{
								...
								items : [[editorInstance.lang.autosave.inline, 'inline'], [editorInstance.lang.autosave.sideBySide, 'sideBySide']],
								//'default': 'sideBySide',
								'default' : 'inline', // EDIT
								onClick : function () {
	...
	function CheckForAutoSavedContent(editorInstance, autoSaveKey, notOlderThan) {
		...
		if (localStorage.getItem(autoSaveKey)) {
			...
			var confirmMessage = editorInstance.lang.autosave.loadSavedContent.replace("{0}", format_date(new Date(moment(autoSavedContentDate))));
	...
	function RenderDiff(dialog, editorInstance, autoSaveKey) {
		...
		dialog.getContentElement('general', 'diffContent').getElement().setHtml('<div class="diffContent">' + diffview.buildView({
				...
				newTextName : editorInstance.lang.autosave.autoSavedContent + format_date(new Date(moment(jsonSavedContent.saveTime))), // EDIT
				contextSize : 3,
